...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        world_id = torch.distributed.get_rank()
        local_id = torch.distributed.get_rank() - args.local_rank * 4
    ...
    logging("rank {} device {} n_gpu {} distributed training {}".format(world_id, device, n_gpu, bool(args.local_rank != -1)))
    ...
    train_data = None
    if args.do_train:
        train_data = data_util.Loader(args.data_dir, data_file['train'], args.cache_size, args.train_batch_size, args.local_rank != -1, world_size, world_id, device)
        train_data_num = train_data.data_num // world_size
        train_data = train_data.data_iter()
        #sampler = DistributedSampler(train_data)
        #loader = DataLoader(train_data, batch_size=args.train_batch_size, sampler=sampler)
        # pdb.set_trace()
        num_train_steps = int(
            train_data_num / args.train_batch_size / args.gradient_accumulation_steps * args.num_train_epochs)
    ...
    if args.local_rank != -1:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[local_id],
                                                          output_device=local_id, find_unused_parameters=True)
    elif n_gpu > 1:
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        valid_data = data_util.Loader(args.data_dir, data_file['dev'], args.cache_size, args.eval_batch_size, device=device, ddp_train=False)
