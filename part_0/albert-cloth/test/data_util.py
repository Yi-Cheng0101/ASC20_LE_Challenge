...
def get_json_file_list(data_dir):
    ...
    for root, dir_names, file_names in os.walk(data_dir):
        for filename in sorted(fnmatch.filter(file_names, '*.json')):
...
if __name__ == '__main__':
    ...
    data_collections = ['temp']
    for item in data_collections:
        args.data_dir = '/home/engine210/LE/dataset/ELE/{}'.format(item)
        args.pre = args.post = 0
        args.bert_model = 'albert-xxlarge-v2'
        args.save_name = '{}-{}.pt'.format(item, args.bert_model)
