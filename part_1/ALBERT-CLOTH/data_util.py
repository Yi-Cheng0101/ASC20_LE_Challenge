...
class Loader(object):
    def __init__(self, data_dir, data_file, cache_size, batch_size, ddp_train, world_size=-1, world_id=-1, device='cpu'):
        #self.tokenizer = BertTokenizer.from_pretrained(args.bert_model)
        ...
        if self.ddp_train:
            ...
            while (cache_start < self.data_num):
                ...
                while (batch_start + cache_start < cache_end):
                    ...
                    data_list.append((inp, tgt))
                    batch_start += self.batch_size
                cache_start += self.cache_size
            data_list_len = len(data_list)
            data_slice_len = data_list_len // self.world_size
            #data_len_mod = data_list_len % self.world_size
            data_slice = data_list[self.world_id * data_slice_len : (self.world_id + 1) * data_slice_len]
            '''
            if self.world_id < data_len_mod:
                data_slice = data_list[self.world_id * (data_slice_len + 1) : (self.world_id + 1) * (data_slice_len + 1)]
            else:
                data_slice = data_list[self.world_id * data_slice_len + data_len_mod: (self.world_id + 1) * data_slice_len + data_len_mod]
            '''
            for i in data_slice:
                inp, tgt = i
...
if __name__ == '__main__':
    ...
    data_collections = ['multi_dev', 'single_dev']
    for item in data_collections:    
        args.data_dir = './split_dev/{}'.format(item)
